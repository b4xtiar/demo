  ...
  <div>
    <v-content>
      <v-layout row wrap style="direction:rtl">
        <v-subheader>بەروار دیاری بکە</v-subheader>
        <v-flex xs12 style="direction:ltr">
          <v-date-picker v-model="selecteddate" full-width mx-2 class="mt-3" color="blue darken-4" first-day-of-week="6">
          </v-date-picker>
        </v-flex>
      </v-layout>
      <v-layout row wrap style="direction:rtl" mt-2>
        <v-flex xs12 sm12>
          ...
          <v-card dark color="orange darken-4" class="mt-2">
            <v-card-title primary-title>
              ئاماری فرۆشتن
            </v-card-title>
            <v-card-text>
                    ...
                    <v-layout row wrap justify-space-around>
                      <v-flex xs12 sm12 md12 lg12 style="direction:rtl;text-align:right">
                        <v-card v-for="(w,i) in sells" :key="i" class="mt-2 elevation-5">
                          <v-card-title primary-title v-for="(item, index) in dukan" :key="index" v-if="item.id == w.dukan">
                            {{item.name}}
                          </v-card-title>
                          <v-card-text>
                            <v-layout row wrap>
...
<script>
  ...
  export default {
    data() {
      return {
        selecteddate: new Date().getFullYear() + '-' + ('0' + (new Date().getMonth() + 1)).slice(-2) + '-' + ('0' +
